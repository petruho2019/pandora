...
import {
  ...
  loadRequestsSuccess,
  openRequestInFS,
  openRequestInFSFailure,
  openRequestInFSSuccess,
...
import {
  ...
  renameRequestSuccess,
  updateRequest,
  updateRequestSuccess,
} from '../actions/modal-actions/request-modal.actions';
...
export class RequestEffects {
        ...
        from(this.electronService.createRequest({ requestInfo: actionData.body })).pipe(
          map((createRequestResult) => {
            if (createRequestResult.isSuccess) {
              ...
              console.log(
                `addHttpRequestSuccess логирование объекта ${JSON.stringify(createRequestResult)}`,
              );
              return createRequestSuccess({ request: createRequestResult.body! });
            } else {
              console.log(
                `addHttpRequestFailure логирование объекта ${JSON.stringify(createRequestResult)}`,
              );
  ...
  loadRequests$ = createEffect(() =>
    this.actions$.pipe(
      ofType(loadRequests),
      concatLatestFrom(({ collectionInfo: collInfo }) =>
        this.store.select(selectLoadedByCollectionId({ collectionId: collInfo.collectionId })),
      ),
      filter(([action, isLoaded]) => !isLoaded),
      switchMap(([{ collectionInfo: collectionInfo }, isLoaded]) => {
        return from(
          this.electronService.loadRequests({
            collectionId: collectionInfo.collectionId,
            collectionPath: collectionInfo.collectionPath,
          }),
        ).pipe(
          map((loadRequestsResult) => {
            if (loadRequestsResult.isSuccess)
              return loadRequestsSuccess({
                loadedRequests: loadRequestsResult.body as RequestModel[],
                collectionId: collectionInfo.collectionId,
              });
            else {
        ...
        return from(this.electronService.renameRequest(actionData.body)).pipe(
          map((renameRequestResult) => {
            if (renameRequestResult.isSuccess) {
              ...
              return renameRequestSuccess({
                renamedRequest: renameRequestResult.body as RequestModel,
              });
            } else {
        ...
        return from(this.electronService.openRequestInFS(requestInfo)).pipe(
          map((openRequestInFsResult) => {
            ...
            if (openRequestInFsResult.isSuccess) return openRequestInFSSuccess();
              ...
              return openRequestInFSFailure({
                errorMessage: openRequestInFsResult.errorMessage as string,
              });
            }
          }),
          catchError((err) => {
