...
export class CollectionItem {
  ...
  @Output() handleAddRequest = new EventEmitter<{
    overlay: OverlayRef;
    addRequestInfo: CreateRequestInfo;
  }>();
  @Output() handleCloneCollection = new EventEmitter<{
    overlay: OverlayRef;
    cloneCollectionDto: CloneCollectionDto;
  }>();
  @Output() handleRenameCollection = new EventEmitter<{
    overlay: OverlayRef;
    renameDto: RenameDto;
  }>();
  @Output() handleCloseCollection = new EventEmitter<{
    overlay: OverlayRef;
    collectionId: string;
  }>();
    ...
    this.actionsMenuService.openedId$.pipe(take(1)).subscribe((current) => {
      ...
      current === id
        ...
        : this.actionsMenuService.open(
            id,
            actionsTrigger,
            this.collectionActionMenuPortal(),
  ...
  cloneCollection(collectionInfo: CloneCollectionDto) {
    this.handleCloneCollection.emit({
      overlay: this.cloneCollectionOverlayRef,
      cloneCollectionDto: collectionInfo,
    });
  }

  renameCollection(collectionInfo: RenameDto) {
    this.handleRenameCollection.emit({
      overlay: this.renameCollectionOverlayRef,
      renameDto: collectionInfo,
    });
  }

  removeCollection(collectionId: string) {
    this.handleCloseCollection.emit({
      overlay: this.removeCollectionOverlayRef,
      collectionId: collectionId,
    });
  }
