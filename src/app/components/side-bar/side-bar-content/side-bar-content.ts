...
export class SideBarContent {
  ...
  ngOnInit(): void {
    ...
    this.openCollections.set(
      (this.collections() as Collection[]).reduce(
        (acc, c) => {
          acc[c.id] = false;
          return acc;
        },
  ...
  handleCreateRequest(overlay: OverlayRef, request: CreateRequestInfo) {
    switch (request.type) {
      case RequestTypes.HTTP:
        console.log(
          `onCreateRequest addHttpRequest: ${JSON.stringify(request)} , collectionId ${this.actionsMenuService.currentId}`,
        );
        this.store.dispatch(
          createHttpRequest({
            actionData: {
              body: request,
              modalOverlayRefs: [overlay],
              successMessage: 'Запрос успешно добавлен',
            },
          }),
        );
        break;
  ...
  handleCloneCollection(overlay: OverlayRef, collectionInfo: CloneCollectionDto) {
    this.store.dispatch(
      cloneCollectionModal({ actionData: { modalOverlayRefs: [overlay], body: collectionInfo } }),
    );
  }
  ...
  handleOpenCollection(collectionId: string, isOpen: boolean) {
    ...
    if (requestsByCollectionId === undefined || !requestsByCollectionId.isLoaded) {
      this.loadRequestsByCollectionId(
        collectionId,
        this.collections()?.find((c) => c.id === collectionId)?.path as string,
      );
      return;
    }
  }
  ...
  handleCloseCollection(overlay: OverlayRef, collectionId: string) {
    this.store.dispatch(
      closeCollectionModal({
        actionData: { modalOverlayRefs: [overlay], body: { collectionId: collectionId } },
      }),
    );
  }
  ...
  loadRequestsByCollectionId(collectionId: string, collectionPath: string) {
    this.store.dispatch(
      loadRequests({
        collectionInfo: { collectionPath: collectionPath, collectionId: collectionId },
      }),
    );

    this.store
      .select(selectRequestsByCollectionId({ collectionId: collectionId }))
      .subscribe((reqs) => {
    ...
    if (collectionId === 'dc378aa8-b42e-468a-bb5d-5dad6e0f9b7b') {
      this.requests.update((state) => ({
        ...
        [collectionId]: {
          isLoaded: true,
          requests: Object.entries(INITIAL_REQUESTS_STATE.entities)
            .filter((r) => r[1]!.collectionId === collectionId)
            .map((r) => r[1] as RequestModel),
        },
      }));
    }
  ...
  closeCollectionRef(collInfo: {
    collectionId: string;
    collectionName: string;
    collectionPath: string;
  }) {
    this.collectionItem.showCloseCollectionModal(
      collInfo.collectionId,
      collInfo.collectionName,
      collInfo.collectionPath,
    );
  }

  dropCollection($event: CdkDragDrop<string[]>) {
    this.store.dispatch(
      moveCollection({ fromIndex: $event.previousIndex, toIndex: $event.currentIndex }),
    );
  }
