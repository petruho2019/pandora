...
export function initializeRequest(store: ElectronStore<RequestsElectronSchema>, ipcMain: IpcMain) {
  ...
  ipcMain.handle(
    ...
    ): Promise<ResultT<HttpRequestModel, string>> => {
      try {
        ...
        const savedRequest: RequestModel =
          requestInfo.id !== null
            ? mapCreateRequestModel(requestInfo, requestInfo.id)
            : mapCreateRequestModel(requestInfo, uuidv4());
  ...
  ipcMain.handle(
    ...
    async (event, collectionInfo: LoadRequestDto): Promise<ResultT<RequestModel[], string>> => {
      ...
      try {
        requestsFileInfo = await fs.promises.readdir(collectionInfo.collectionPath, {
          withFileTypes: true,
        });
      } catch (error) {
        ...
        if (fileInfo.name !== COLLECTION_CONFIG_FILE_NAME && fileInfo.isFile()) {
          try {
            const raw = JSON.parse(
              await fs.promises.readFile(path.join(collectionInfo.collectionPath, fileInfo.name), {
                encoding: 'utf8',
              }),
            ) as RequestModel;
            ...
            if (error instanceof ZodError) {
              return buildFailureResultT(
                `Ошибка при загрузке файла ${fileInfo.name} , ${error.message}`,
              );
            }
            if (error instanceof SyntaxError) {
              return buildFailureResultT(
                `Невалидный JSON в файле ${fileInfo.name} , ${error.message}`,
              );
            }
    ...
    async (event, requestInfo: RenameRequestDto): Promise<ResultT<RequestModel, string>> => {
      ...
      try {
        await fs.promises.rename(
          path.join(requestInfo.collectionPath, requestInfo.oldFileName + '.json'),
          path.join(requestInfo.collectionPath, requestInfo.newFileName + '.json'),
        );
      } catch (err: any) {
        if (err.code === 'ENOENT') {
      ...
      requestFromStore.splice(
        requestFromStore.findIndex((c) => c.id === requestInfo.requestId),
        1,
        requestById,
      );
  ...
  ipcMain.handle(
    ...
    async (event, requestInfo: CloneRequestDto): Promise<ResultT<RequestModel, string>> => {
      ...
      try {
        console.log(
          `Trying to write file by path: ${path.join(requestInfo.collectionPath!, requestInfo.newFileName)}`,
        );
        await fs.promises.writeFile(
          path.join(requestInfo.collectionPath!, requestInfo.newFileName) + '.json',
          JSON.stringify(clonedRequest, null, 2),
          { encoding: `utf-8` },
        );
      } catch (err) {
  ...
  ipcMain.handle(
    'open-request-in-fs',
    async (event, requestInfo: OpenRequestInFSDto): Promise<Result> => {
      ...
      fs.stat(fullPath, (err, stat) => {
        if (err) {
          if (err.code === 'ENOENT')
            return buildFailureResultT('Запрос не найден в файловой системе');
        }
...
function mapCloneDtoToRequestModel(
  baseRequest: RequestModel,
  cloneRequestDto: CloneRequestDto,
): RequestModel {
...
function mapCreateRequestModel(requestInfo: CreateRequestInfo, id: string): HttpRequestModel {
